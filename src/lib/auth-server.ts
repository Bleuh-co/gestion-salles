...
/**
 * Résout l'ID Firestore de l'app Gestion Salles dans la collection `apps`.
 *
 * Priorité 1 : variable d'env GESTISALLE_APP_ID (direct, zéro ambiguïté).
 * Priorité 2 : matcher par nom dans la collection apps (fallback dev).
 */
async function resolveGestiSalleAppId(
  db: FirebaseFirestore.Firestore
): Promise<{ appId: string; appName: string | null }> {
  const override = process.env.GESTISALLE_APP_ID;
  if (override) {
...
export async function resolveRoleVerbose(email: string): Promise<RoleResolution> {
  ...
  try {
    const { appId, appName } = await resolveGestiSalleAppId(db);
    base.appId = appId;
